...
agent:
  ...
  require_argv: ["--model"]`}</pre></figure></section>
          ...
          <section id="adoption" className="docs-section adoption-section">{sectionMarker("adoption", "05", t.adoption.label)}<h2>{t.adoption.title}</h2><div className="adoption-list">{t.adoption.steps.map(([number, title, text]) => <article key={number}><span>{number}</span><div><h3>{title}</h3><p>{text}</p></div><GitBranch size={18} /></article>)}</div></section>
          <DossierBand index="06" locale={locale} en="RELEASE RECORD / OWNED CHANGE" vi="HỒ SƠ RELEASE / THAY ĐỔI CÓ OWNER" />
          <section id="operations" className="docs-section">{sectionMarker("operations", "06", t.operations.label)}<h2>{t.operations.title}</h2><p className="docs-lead">{t.operations.text}</p><div className="ops-ledger">{t.operations.rows.map(([label, text]) => <div key={label}><Settings2 size={15} /><code>{label}</code><span>{text}</span></div>)}</div></section>
