...
import { ArrowUpRight, BookOpen, CheckCircle2, ChevronRight, FileCode2, GitBranch, Layers3, Link2, Network, Search, Settings2, ShieldCheck, Workflow } from "lucide-react";
...
function DocsContent({ locale }: { locale: Locale }) {
  ...
  const scrollTo = (id: string, updateHash = true) => { document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" }); if (updateHash) window.history.pushState(null, "", `#${id}`); };
  const searchIndex = useMemo(() => [
    { id: "start", title: t.start.title, text: `${t.start.lead} ${t.start.questions.flat().join(" ")}`, keywords: "control evidence governance intent record" },
    { id: "foundation", title: t.foundation.title, text: `${t.foundation.text} ${t.foundation.cards.flat().join(" ")}`, keywords: "policy data deterministic artifact graph rust" },
    { id: "lifecycle", title: t.lifecycle.title, text: `${t.lifecycle.text} ${t.lifecycle.stages.flat().join(" ")}`, keywords: "requirement specification task test release traceability" },
    { id: "kit", title: t.kit.title, text: t.kit.text, keywords: "spec kit yaml markdown validator" },
    { id: "validation", title: t.validation.title, text: `${t.validation.text} ${t.validation.controls.flat().join(" ")}`, keywords: "finding rule severity launch exception" },
    { id: "adoption", title: t.adoption.title, text: t.adoption.steps.flat().join(" "), keywords: "team baseline onboarding roles" },
    { id: "operations", title: t.operations.title, text: `${t.operations.text} ${t.operations.rows.flat().join(" ")}`, keywords: "release compatibility rollback review" },
    { id: "reference", title: t.reference.title, text: t.reference.cards.flat().join(" "), keywords: "id grammar configuration cli glossary" },
  ], [t]);
  const searchResults = useMemo(() => { const term = query.trim().toLocaleLowerCase(); return term ? searchIndex.filter((entry) => `${entry.title} ${entry.text} ${entry.keywords}`.toLocaleLowerCase().includes(term)) : []; }, [query, searchIndex]);
  useEffect(() => { const followHash = () => { const id = decodeURIComponent(window.location.hash.slice(1)); if (searchIndex.some((entry) => entry.id === id)) window.setTimeout(() => scrollTo(id, false), 20); }; followHash(); window.addEventListener("hashchange", followHash); return () => window.removeEventListener("hashchange", followHash); }, [searchIndex]);
  const copyLink = async (id: string) => { await navigator.clipboard?.writeText(`${window.location.origin}${window.location.pathname}#${id}`); };
  const sectionMarker = (id: string, number: string, label: string) => <div className="section-marker"><span>{number}</span><span>{label}</span><button onClick={() => copyLink(id)} aria-label={locale === "en" ? `Copy link to ${label}` : `Sao chép link đến ${label}`} title={locale === "en" ? "Copy section link" : "Sao chép link section"}><Link2 size={14} /></button></div>;

  return (
    <div className="docs-page">
      <section className="docs-hero"><div className="docs-hero-grid" aria-hidden="true" /><div><div className="eyebrow"><span className="pulse-line" /> {t.eyebrow}</div><h1>{t.title}</h1><p>{t.lead}</p><div className="docs-hero-seal"><ShieldTraceMark /><div><span>GOVERNED DOSSIER / DOCS-02</span><strong>{locale === "en" ? "Evidence-bound delivery manual" : "Manual delivery gắn với evidence"}</strong></div><i className="verify-dot" aria-hidden="true" /></div><div className="docs-search"><label htmlFor="docs-search">{locale === "en" ? "SEARCH THE CONTROLLED RECORD" : "TÌM TRONG HỒ SƠ CÓ KIỂM SOÁT"}</label><div className="docs-search-box"><Search size={16} /><input id="docs-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={locale === "en" ? "Search policy, evidence, traceability, lifecycle…" : "Tìm policy, evidence, traceability, lifecycle…"} autoComplete="off" /><span>{query ? `${searchResults.length} / 08` : "08 / 08"}</span></div>{query && (searchResults.length ? <ul className="docs-search-results">{searchResults.map((entry, index) => <li key={entry.id}><a href={`#${entry.id}`} onClick={() => { scrollTo(entry.id); setQuery(""); }}><code>{String(index + 1).padStart(2, "0")}</code><div><strong>{entry.title}</strong><p>{entry.text}</p></div><ArrowUpRight size={15} /></a></li>)}</ul> : <p className="docs-search-empty">{locale === "en" ? "No controlled record matches this search." : "Không có controlled record khớp với truy vấn."}</p>)}</div><div className="docs-route-actions"><a className="ink-button" href="/policies">{t.action} <ArrowUpRight size={16} /></a><a className="underlined-button" href="/quickstart">{locale === "en" ? "Role quickstarts" : "Quickstart theo vai trò"} <ArrowUpRight size={15} /></a><a className="underlined-button" href="/releases">{locale === "en" ? "Change & compatibility" : "Thay đổi & tương thích"} <ArrowUpRight size={15} /></a></div></div><div className="docs-facts">{t.facts.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>
      <div className="docs-body">
        ...
        <div className="docs-content">
          ...
          <section id="start" className="docs-section">{sectionMarker("start", "00", t.start.label)}<h2>{t.start.title}</h2><p className="docs-lead">{t.start.lead}</p><div className="question-grid">{t.start.questions.map(([number, title, text]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div><div className="reader-paths">{t.start.readerPaths.map(([role, path]) => <div key={role}><code>{role}</code><span>{path}</span></div>)}</div></section>
          <section id="foundation" className="docs-section docs-model"><div className="docs-section-copy">{sectionMarker("foundation", "01", t.foundation.label)}<h2>{t.foundation.title}</h2><p>{t.foundation.text}</p></div><div className="foundation-grid">{t.foundation.cards.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
          <DossierBand index="02" locale={locale} en="TRACE PATH / INTENT → RECORD" vi="ĐƯỜNG TRACE / INTENT → RECORD" />
          <section id="lifecycle" className="docs-section">{sectionMarker("lifecycle", "02", t.lifecycle.label)}<h2>{t.lifecycle.title}</h2><p className="docs-lead">{t.lifecycle.text}</p><div className="lifecycle-manual">{t.lifecycle.stages.map(([number, title, text], index) => <article key={number}><span>{number}</span><Workflow size={16} /><h3>{title}</h3><p>{text}</p>{index < t.lifecycle.stages.length - 1 && <i aria-hidden="true" />}</article>)}</div></section>
          <section id="kit" className="docs-section kit-manual"><div>{sectionMarker("kit", "03", t.kit.label)}<h2>{t.kit.title}</h2><p>{t.kit.text}</p><div className="kit-icons"><span><FileCode2 size={18} /> YAML rule packs</span><span><Layers3 size={18} /> Markdown contracts</span><span><ShieldCheck size={18} /> Rust validator</span></div></div><figure className="docs-code"><figcaption>{t.kit.caption}</figcaption><pre>{`kit:
  version: "^1.0.0"
...
agent:
  ...
  require_argv: ["--model"]`}</pre></figure></section>
          ...
          <section id="validation" className="docs-section">{sectionMarker("validation", "04", t.validation.label)}<h2>{t.validation.title}</h2><p className="docs-lead">{t.validation.text}</p><div className="control-board">{t.validation.controls.map(([label, title, text]) => <article key={label}><code>{label}</code><div><h3>{title}</h3><p>{text}</p></div><CheckCircle2 size={17} /></article>)}</div><a href="/policies" className="underlined-button">{locale === "en" ? "Inspect current policy controls" : "Kiểm tra policy control hiện tại"} <ArrowUpRight size={15} /></a></section>
          <section id="adoption" className="docs-section adoption-section">{sectionMarker("adoption", "05", t.adoption.label)}<h2>{t.adoption.title}</h2><div className="adoption-list">{t.adoption.steps.map(([number, title, text]) => <article key={number}><span>{number}</span><div><h3>{title}</h3><p>{text}</p></div><GitBranch size={18} /></article>)}</div><a href="/quickstart" className="underlined-button">{locale === "en" ? "Open role-based CLI quickstarts" : "Mở quickstart CLI theo vai trò"} <ArrowUpRight size={15} /></a></section>
